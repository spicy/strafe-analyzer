...
static struct {
	zgui::color window_border_inner_fill{ 10, 14, 23, 255 };
	zgui::color window_border_fill{ 10, 14, 23, 255 };
	//zgui::color window_border_color{ 13, 16, 22, 255 };
	//zgui::color window_background{ 152, 240, 0, 255 };

	zgui::color control_outline{ 8, 11, 16, 255 };

	zgui::color window_border_background{ 8, 11, 16, 255 };

	zgui::color control_active_or_clicked{ 237, 179, 64, 255 };
	zgui::color control_active_or_clicked_textbox{ 18, 25, 36, 255 };
	zgui::color control_idle{ 10, 14, 20, 255 };

	zgui::color color_groupbox_bg{ 8, 11, 16, 255 };
	zgui::color color_text{ 200, 200, 202, 255 };
	zgui::color color_text_dimmer{ 80, 95, 112, 255 };
	zgui::color color_slider{ 237, 179, 64, 255 };
	zgui::color color_combo_bg{ 237, 179, 64, 255 };
} global_colors;
...
static struct {
	...
	// Key that will toggle gathering tick history
	int pause_toggle_key = VK_F11;
} global_config;
...
bool zgui::begin_window(std::string_view title, const vec2 default_size, const unsigned long font, const int flags)
{
	...
	if (key_pressed(global_config.pause_toggle_key))
		g_menu.strafetrainer.paused = !g_menu.strafetrainer.paused;
	...
	if (context.window.opened) menu_open = true;
	else menu_open = false;


	if (const int prev_alpha = context.window.alpha; !(flags & zgui_window_flags_no_ontoggle_animation))
	{
		const int fade_factor = static_cast<int>(1.0f / 0.15f * functions.get_frame_time() * 255);
...
void zgui::begin_groupbox(std::string_view title, const vec2 size, const int flags) noexcept
{
	...
	const vec2 draw_pos{ context.window.position.x + cursor_pos.x - 10, context.window.position.y + cursor_pos.y - 7 };
...
bool zgui::tab_button(const char* id, const vec2 size, bool value) noexcept
{
	...
	const vec2 cursor_pos = pop_cursor_pos();
	const vec2 draw_pos{ context.window.position.x + cursor_pos.x - 10, context.window.position.y + cursor_pos.y };
	...
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1 }, zgui_render_type::zgui_filled_rect, global_colors.window_border_fill, "",{size.x - 2, size.y - 2} });
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, global_colors.window_border_fill, "", size });
...
void zgui::text_input(const char* id, std::string& value, const int max_length, const int flags) noexcept
{
	...
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1 }, zgui_render_type::zgui_filled_rect,  active ? global_colors.control_active_or_clicked_textbox : global_colors.control_idle, "", { control_width - 2, control_height - 2 } });
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, global_colors.control_outline,"", { control_width, control_height } });
...
void zgui::slider_int(const char* id, const int min, const int max, int& value, const int horizSize) noexcept
{
	std::vector<std::string> id_split = split_str(id, '#');

	const int control_width = horizSize; //normally 150
	...
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, global_colors.control_idle,"", { static_cast<float>(control_width) - 2, static_cast<float>(control_height) - 2 } });
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, global_colors.control_outline,"", { static_cast<float>(control_width), static_cast<float>(control_height) } });
...
bool zgui::clickable_text(const char* id) noexcept
{
	...
	const bool clicked = key_pressed(VK_LBUTTON);

	if (!active && hovered && clicked)
	{
		context.window.blocking = hash(id);
	}
